 ...
 *
 * $Id: dhd_linux.c 323022 2012-03-22 17:48:58Z $
 */
...
#if defined(CONFIG_COMMON_PATCH) && defined(WLP2P)
uint dhd_init_ap_val=1;
...
#if defined(CONFIG_LGE_BCM432X_PATCH)
int dhd_dev_reset(struct net_device *dev, uint8 flag);	/* LGE_patch */
#endif
...
{	
	...
	if(IS_ERR(fp)) 
	{
		...
		DHD_ERROR(("%s: set fw path to default(STA).\n", __FUNCTION__));
		strcpy(firmware_path,"/etc/firmware/fw_bcmdhd.bin");
//		firmware_path[0] = '\0';
		fp = NULL;
		ret = -1;
		goto exit;
...
exit:
	if (fp) filp_close(fp, current->files);
...
#if defined(CONFIG_COMMON_PATCH) && defined(WLP2P)
	ret = dhd_wl_ioctl_cmd(dhd, WLC_GET_AP, (char *)&dhd_init_ap_val, sizeof(dhd_init_ap_val), FALSE, 0);
	if (!ret) {
		DHD_ERROR(("Initial WLC_GET_AP value is [%d]\n", dhd_init_ap_val));
	}
	else {
		DHD_ERROR(("Failed WLC_GET_AP get command\n"));
	}
...
#if defined(CONFIG_LGE_BCM432X_PATCH)	/* LGE_patch : config file setting */
