...
#if defined(CONFIG_COMMON_PATCH) && defined(WLP2P)
		if (dhd_init_ap_val==0) {
			val=1;
			wldev_ioctl(_ndev, WLC_DOWN, &val, sizeof(s32), true);
			if ((err = wldev_ioctl(_ndev, WLC_SET_AP, &val, sizeof(s32), true)) < 0)
				WL_ERR(("setting AP mode failed %d \n", err));
			val=0;
			wldev_ioctl(_ndev, WLC_UP, &val, sizeof(s32), true);
		}
#endif /* defined(CONFIG_COMMON_PATCH) && defined(WLP2P) */
...
wl_cfg80211_del_virtual_iface(struct wiphy *wiphy, struct net_device *dev)
{
	...
	if (wl->p2p_supported) {
		...
		if(dhd_init_ap_val==0) {
			val=1;
			wldev_ioctl(dev, WLC_DOWN, &val, sizeof(s32), true);
			val=0;
			wldev_iovar_setint(dev, "apsta", val);
			if ((err = wldev_ioctl(dev, WLC_SET_AP, &val, sizeof(s32), true)) < 0)
				WL_ERR(("setting AP 0 failed %d \n", err));
			err=0;
			val=1;
			wldev_iovar_setint(dev, "apsta", val);
			val=0;
			wldev_ioctl(dev, WLC_UP, &val, sizeof(s32), true);
		}
#endif
...
#if defined (BCMWAPI_WPI) || defined (BCMCCX)
	...
	const wl_event_msg_t *e, void *data)
{
	...
	if (band == NULL)
	{
		WL_ERR(("P:%s:invalid band is selected, band is NULL:channel[%d]\n", __FUNCTION__, channel));
		err = WL_INVALID;
		goto exit;
	}
#endif
